...
fn parse_cli() -> CliArgs {
    ...
    let matches = clap::App::new("rproxy")
    .version("0.1.0")
    .author("Guy Bortnikov, Shay Sandler")
    .arg(clap::Arg::with_name(target_arg_name)
        .help("target program (binary) to run under rproxy")
        .required(true)
        .index(1))
    .arg(clap::Arg::with_name(config_arg_name)
        .help("configuration path, default will be taken 'rproxy.conf' from the current directory")
        .required(false)
        .index(2)
